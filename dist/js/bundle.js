...
function form() {
  ...
  formContact.addEventListener('submit', function (event) {
    ...
    var formData = new FormData(formContact); //То, что ввел пользователь в форму
    //Отправка данных в json формате

    var obj = {}; //Превратили объект formData в обычный читаемый объект

    formData.forEach(function (value, key) {
      obj[key] = value;
    });
    var json = JSON.stringify(obj);
    request.send(json); //Отправили

    request.addEventListener('readystatechange', function () {
      // Отслеживание состояний запроса
...
function modal() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    request.setRequestHeader('Content-Type', 'application/json; charset=utf-8');
    var formData = new FormData(form); //То, что ввел пользователь в форму
...
function slider() {
  //Слайдер
  var slideIndex = 1; //Переменная отвечает за тот слайд, к-ый показывается на странице
...
function tabs() {
  var tab = document.querySelectorAll('.info-header-tab'); //получаем все кнопки

  var info = document.querySelector('.info-header'); //получаем родителя кнопок

  var tabContent = document.querySelectorAll('.info-tabcontent'); //получаем контент, который будем скрывать

  function hideTabContent(a) {
    // функция скрывает весь контент, кроме первого
  ...
  function showTabContent(b) {
    //функция показывает определенный  контент
    if (tabContent[b].classList.contains('hide')) {
      //проверям скрыт ли элемент
  ...
  info.addEventListener('click', function (event) {
    //используем делегирование на родителе
    var target = event.target; //получаем событие

    if (target && target.classList.contains('info-header-tab')) {
      // если событие клика совпадает по классу с нашими кнопками, то
      for (var i = 0; i < tab.length; i++) {
        if (target == tab[i]) {
          // проверяем кнопку с контентом для нее
...
function timer() {
  ...
  function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()); // Вычисляем разницу между дедлайном и настоящим временем (мс)

    var seconds = Math.floor(t / 1000 % 60); //получаем секунды из милисекунд 

    var minutes = Math.floor(t / 1000 / 60 % 60); //получаем минуты

    var hours = Math.floor(t / (1000 * 60 * 60)); //получаем часы
